...
That gap is `UnnameableScriptTest`, and it is here because the mutation found
it, not because the review did.

**T7 — the claim nobody executed.** Seven defects were later found in this
...
assignment; a command value nobody ever typed never had to be typable. The
...
class StackDetectionTest(ScanFixture):
    ...
    def stacks(self, survey):
        return [name for name, _declaration in survey.stacks]
    ...
    def test_the_evidence_is_the_declaration_that_established_it(self):
        ...
        self.node_fixture()
        self.assertEqual([("node", "package.json")], list(self.survey().stacks))
    ...
    def test_a_nested_declaration_is_not_a_root_stack(self):
        """The interim boundary (DECISION-2026-08-14): root only."""
...
class DuplicateProposalTest(ScanFixture):
    ...
    def test_a_target_declared_in_two_makefiles_is_proposed_once(self):
        self.write("Makefile", "test:\n\techo a\n")
        self.write("GNUmakefile", "test:\n\techo b\n")
        self.commit()
        self.assertEqual(["make test"], self.values(self.survey().commands))

    def test_the_fixture_really_declares_it_twice(self):
        """Non-vacuity: the equality above would also hold if one of the two
        Makefiles were never read at all."""
        self.write("Makefile", "test:\n\techo a\n")
...
class RootBoundaryTest(ScanFixture):
    ...
    def test_a_nested_package_manifest_is_diagnosed_by_name(self):
        ...
        self.assertEqual(["packages/web/package.json"], [n.where for n in notes])
...
class UnparseableDeclarationTest(ScanFixture):
    ...
    def test_a_task_runner_declaration_proposes_nothing_and_says_why(self):
        ...
        self.assertEqual(["Taskfile.yml"], [n.where for n in notes])

    def test_pyproject_is_detected_as_python_and_read_by_nobody(self):
        self.python_fixture()
        survey = self.survey()
        self.assertIn("python", [name for name, _d in survey.stacks])
...
class UnnameableScriptTest(ScanFixture):
    """P3.2 — a name the grammar cannot concatenate is diagnosed, not mangled.

    **Found by a mutation run, not by review**: deleting the whitespace check
    from `_name_fault` reddened nothing, so the check was live but unasserted.

    The grammar composes `npm run <name>`. A script called `build all` would
    become `npm run build all`, which npm reads as `npm run build` with an
    argument — a command record naming something this repository does not
    declare, which is A1 manufactured by us rather than detected. A name
    carrying an ASCII control character is worse: the value would render as two
    lines of a one-item-per-line section, and `manifest.validate` would refuse
    it at exit 2 (ADR-32).

    **The whitespace half is now a choice rather than a necessity, and it is
    kept.** `scan._shell_word` would quote the name (`npm run 'build all'`), so
    the value would be typable — but widening what a scanner proposes is a
    behaviour change nobody asked for, and a refusal carrying a diagnostic is
    never a false claim. The control-character half is not a choice: no record
    can hold one.
    """
    ...
    def test_a_script_name_with_whitespace_is_not_proposed(self):
    ...
    def test_the_diagnostic_names_the_declaration_and_the_name(self):
        ...
        self.assertEqual([PACKAGE_JSON], [n.where for n in notes])
        self.assertIn(repr("build all"), notes[0].observed)

    def test_an_ordinary_name_is_still_proposed(self):
        """The control: without it, a `_name_fault` that rejected everything
...
class MakefileGrammarTest(ScanFixture):
    ...
    def test_the_first_makefile_name_git_tracks_is_the_one_read(self):
        """`GNUmakefile` wins over `Makefile` for GNU make, and the-steward
        reads whichever names it tracks — both, if both are tracked, because
        each is a declaration and neither is silently preferred."""
        self.write("Makefile", "a:\n\techo\n")
        self.write("GNUmakefile", "b:\n\techo\n")
        self.commit()
        self.assertEqual(
            ["make a", "make b"], self.values(self.survey().commands)
        )
...
class TypedCommandTest(ScanFixture):
    ...
    * The argv is asserted, not the exit status. `npm run a;id` exits **0**
    ...
    # `EXECUTABLE_PREFIX` already neutralises.
    EXECUTABLES = (
    ...
    # What a correct scan must produce when each of its values is typed. Ten
    # lines: seven executables, two package scripts, one Makefile target. The
    # option-like names in the fixture (`--help`, `-j4`) are deliberately
    # absent — quoting cannot rescue them, so they are diagnosed instead, which
    # `OptionLikeNameTest` pins.
    EXPECTED_OUTPUT = (
        "BACKSLASH_RAN",
        "DASH_RAN",
        "MAKE-ARGV: [release]",
    ...
    def hostile_repository(self):
        self.write(
            PACKAGE_JSON,
            json.dumps(
                {"scripts": {"check": "tsc --noEmit", "a;id": "x", "--help": "y"}}
            )
            + "\n",
        )
        self.write("Makefile", "release:\n\techo\n\n-j4:\n\techo\n")
...
class OptionLikeNameTest(ScanFixture):
    ...
    def test_the_diagnostic_names_the_name_and_the_file(self):
        ...
        self.assertEqual(["Makefile"], [n.where for n in notes])
...
class SameModeConflictTest(IndexFixture):
    ...
    def test_the_diagnostic_names_both_paths_and_their_stages(self):
        ...
        self.assertEqual(
            ["Makefile", "conflicted.sh"], sorted(n.where for n in notes)
        )
        for note in notes:
...
class WorkingTreeSymlinkDeclarationTest(ScanFixture):
    ...
    def test_a_diagnostic_names_both_replaced_declarations(self):
        ...
        self.assertEqual(
            ["Makefile", PACKAGE_JSON], sorted(n.where for n in notes)
        )
...
class NestedTaskRunnerTest(ScanFixture):
    ...
    def test_every_nested_task_runner_is_named_by_a_diagnostic(self):
        ...
        self.assertEqual(
            sorted(relpath for relpath, _body in self.NESTED),
...
class UnencodableValueTest(IndexFixture):
    ...
    lone surrogates. `_proposable` accepted it; `jsonio.dumps` writes canonical
    ...
    **Platform bound, stated rather than worked around.** APFS refuses such a
    ...
    def test_the_control_shows_those_two_guards_are_not_refusing_everything(self):
        """Without this, a `_proposable` that rejected every value would
