...
//
// `computeDrift` audits UNIVERSALLY: every non-inferred node that is evidenced and carries no `doc`
// claim is UNDOCUMENTED (ADR C-005, C-014, PDR §8). On a synthetic fixture that is exactly right. On
// the first REAL subject — `codex-gate`, 91 nodes — it produced 11 UNDOCUMENTED findings of which
// roughly 2–4 are what a reader means by "a documentation gap"; the rest landed on internal shell
// helpers, and the findings that matter were buried among them.
//
// The tempting fix is to stop DETECTING the internals. It is not available, and not merely
// unattractive: a suppressed UNDOCUMENTED DISAPPEARS. PHANTOM is the opposite cell of the same
// membership grid, and STALE needs an extractor-asserted contradiction record — so no other class can
// recover a finding this one declines to raise. Scoping detection by kind would be a deliberate
// false-negative stage in an audit tool, which is the defect class this initiative has twice treated
// as its most serious (a hollow record that silently deleted a PHANTOM; a defaulted `findings = []`
// that drew a drifting map as clean).
//
...
export const BUCKET_META = Object.freeze({
  'likely-contract': Object.freeze({
    ...
    collapsible: false,
    blurb: 'The subject\'s advertised surface — modes, flags, environment variables and outcomes, plus'
      + ' anything the map itself places at an entry point. A finding here is what a reader usually'
