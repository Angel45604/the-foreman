...
// `quotesFragment` comes from the contract too, so 16b's fixture-hygiene check compares "quotes" the
// one way this repo defines it rather than inventing a second notion (ADR C-006).
import { validate, quotesFragment } from './validate.mjs';
// Test 28's property spans derivation AND rendering — a pointer is only correct if the reader is
// actually sent to the text that needs changing — so the Markdown twin is asserted here rather than
// at a seam nobody tests across.
...
const userMsgClaim = (over = {}) => ({ path: SH, line: 23, text: 'unknown mode; exit 1', claimKind: 'user-message', checked: true, ...over });
const codeEvidence = (over = {}) => ({ path: SH, line: 6, note: 'tiny_core() {', ...over });

/**
 * A COMPLETE documentation harvest over the fixture's one `role: "doc"` source (ADR decision F).
 *
 * Every synthetic below carries it by default, because these nodes exist to exercise the SET
 * MEMBERSHIP rules and decision F is a separate gate in front of one of them: without a complete
 * harvest a node is state 3 — "the map did not look" — and UNDOCUMENTED cannot fire whatever its
 * membership. Making the harvest the default keeps each test below about the rule it was written for;
 * the gate itself is proved on the real artifact in `doc-harvest.test.mjs`.
 */
const completeHarvest = () => ({ searched: [MD], candidates: [] });
...
const node = (kind, label, over = {}) => ({
  ...
  evidence: [], claims: [], docHarvest: completeHarvest(), ...over,
});
...
// Every claim unchecked and NOT ONE of them a `doc` claim — the shape RULE 5 covers and the
// membership grid used to drop on the floor (round-7 gate).
const UNCHECKED_NON_DOC_ONLY = node('flag', 'unchecked non doc only', {
  claims: [commentClaim({ checked: false }), userMsgClaim({ checked: false })],
});
...
/**
 * A `user-message` STALE record — and it CITES A REFUTATION, which took a correction (2026-08-15).
 *
 * It used to set `refutedQuote: 'unknown mode'` against evidence that prints `printf 'unknown mode\n'
 * >&2; exit 2`, so the refuting side REPRODUCED the refuted fragment word for word. C-019 makes `claim`
 * the refuted side and `evidence` the refuting one; evidence that prints the quoted words back is the
 * strongest CONFIRMATION of them, not a contradiction. Its own `statement` gave the game away — "the code
 * also exits 2, which the message never mentions" describes an OMISSION, and an omission is not a
 * contradiction of "unknown mode". `validate()` accepted it because the mechanical half was satisfied
 * (the quote does occur in the claim it cites), which is exactly the half C-019 can check and exactly why
 * a fixture has to carry the other half itself.
 *
 * The record now names a fragment the SAME evidence genuinely refutes: the message tells the user the
 * exit status is 1, and the line cited exits 2. Nothing about the rule moved — only this fixture's
 * quote — and test 16b now fails if any C-019 record here drifts back to quoting what its evidence
 * confirms. The claim text is overridden in BOTH places, because `checkContradictions` requires the
 * record's `claim` to be a subset of one of the node's own `claims[]`.
 *
 * SECOND CORRECTION (2026-08-27, round-6 gate). The claim text was still FABRICATED: it read
 * `'unknown mode; exit 1'` while `run.sh:23` printed only `unknown mode` and exited 2, so a record
 * presented as source-backed IR attributed words to a line that did not carry them. The fixture now
 * carries the staleness for real — line 23 prints `unknown mode; exit 1` and still exits 2 — so
 * `claim.text` and `refutedQuote` are both verbatim from the cited source.
 *
 * WHICH FORCED THE CONTRADICTION'S EVIDENCE NOTE TO NARROW. Once the printed message contains the
 * words `exit 1`, the whole of line 23 REPRODUCES the refuted fragment, and 16b refuses that — it is
 * the one guard that noticed. The contradiction therefore cites the fragment of line 23 that does the
 * refuting, `exit 2` — and the node's own `evidence[0]` is that same citation, because the contract
 * requires a contradiction's evidence to match one of the node's own entries (an unauditable record
 * otherwise). The whole of line 23 is quoted in this comment instead, which is where context belongs.
 */
const STALE_MESSAGE_TEXT = 'unknown mode; exit 1';
const USER_MESSAGE_STALE = node('state', 'user message stale', {
  claims: [userMsgClaim({ text: STALE_MESSAGE_TEXT })],
  evidence: [codeEvidence({ line: 23, note: 'exit 2' })],
  contradictions: [{
    claim: userMsgClaim({ text: STALE_MESSAGE_TEXT }),
    evidence: codeEvidence({ line: 23, note: 'exit 2' }),
    refutedQuote: 'exit 1',
    statement: 'The message tells the user the exit status is 1; the cited line exits 2.',
  }],
});
const TWO_CONTRADICTIONS = node('mode', 'two contradictions', {
  ...
  contradictions: [
    { claim: docClaim({ line: 8, text: 'second documented claim' }), evidence: codeEvidence({ line: 8, note: 'second observation' }), refutedQuote: 'second documented claim', statement: 'The second claim disagrees with line 8.' },
    { claim: docClaim({ line: 7, text: 'first documented claim' }), evidence: codeEvidence({ line: 6, note: 'first observation' }), refutedQuote: 'first documented claim', statement: 'The first claim disagrees with line 6.' },
  ],
});
...
const INFERRED_CONTRADICTION = node('component', 'inferred contradiction', {
  ...
  contradictions: [{
    ...
    evidence: codeEvidence(),
    refutedQuote: 'the docs also assert',
...
const LEGAL_SYNTHETICS = [
  ...
  USER_MESSAGE_STALE, TWO_CONTRADICTIONS, INFERRED_PHANTOM, INFERRED_CONTRADICTION, UNCHECKED_NON_DOC_ONLY,
...
test('8 · RULE 7 — STALE is CLAIM-KIND AGNOSTIC: C-014 narrows documentation, not staleness', () => {
  ...
  // narrowed STALE by claimKind would drop the second — and the real STALE findings on `codex-gate`
  // cover both non-doc kinds: the oracle's one retained STALE is a `user-message` (the overflow
  // message at `codex-gate.sh:519`), and the verified STALE at `:2124` ("Still records the ledger",
  // above `emit_synthetic_approve`) is a `code-comment`. `:2124` was found BY the extractor and is a
  // recorded defect, not a held-out oracle member; the oracle's second STALE was retired by owner
  // decision on 2026-08-13 (ADR C-012 amendment). Both claimKinds must stay representable.
...
test('10b · RULE 5 is about CHECKEDNESS, not claimKind — unchecked NON-doc claims still yield UNVERIFIED', () => {
  // THE HOLE (round-7 gate). `membershipFinding` nested the uncheckable test INSIDE the
  // `docClaims.length > 0` branch, and the other branch needs `evidence.length > 0`. A non-inferred
  // node with no evidence whose claims are all `checked:false` and none of them `doc` therefore
  // matched NEITHER branch and produced no finding at all — a silent miss on a map `validate()`
  // accepts with zero errors, which is the same fail-open family as the three already closed.
  // ADR C-005 conditions UNVERIFIED on the extractor saying it could not check, never on claimKind.
  const m = plus(UNCHECKED_NON_DOC_ONLY);
  assert.deepEqual(validate(m, { repoRoot: REPO_ROOT }).errors, [],
    'precondition: the map the miss happens on is one the contract ACCEPTS');
  assert.deepEqual(classesFor(m, UNCHECKED_NON_DOC_ONLY.id), ['UNVERIFIED'],
    'a node whose every claim is unchecked is UNVERIFIED whatever kind those claims are');
  assert.deepEqual(forNode(m, UNCHECKED_NON_DOC_ONLY.id)[0].citations, UNCHECKED_NON_DOC_ONLY.claims,
    'and it cites every claim, as the doc-claim case already does');
});
...
test('13 · every finding carries the documented shape { class, nodeId, label, detail, citations }', () => {
  ...
  const BASE = ['citations', 'class', 'detail', 'label', 'nodeId'];
  for (const f of findings) {
    // `refutedQuote` is the ONE optional key, and only a STALE finding may carry it: it is copied
    // from the contradiction record that raised the class, so no other class can have one, and a
    // record predating the field leaves even a STALE finding without it.
    const optional = f.class === 'STALE' && 'refutedQuote' in f ? ['refutedQuote'] : [];
    assert.deepEqual(Object.keys(f).sort(), [...BASE, ...optional].sort());
...
test('16b · …and every C-019 record here REFUTES its quote rather than reproducing it', () => {
  // FIXTURE HYGIENE, AND DELIBERATELY NOT A CONTRACT RULE. `validate.mjs` cannot judge whether evidence
  // really refutes a quote and says so in `checkContradictions`'s own note; it checks the mechanical
  // half — that the quote occurs in the claim the record cites. That is the right line and this test
  // does not move it. What it polices is narrower: THIS FILE's fixtures, which exist to exercise C-019
  // and must therefore mean what C-019 means.
  //
  // THE DEFECT IT WAS WRITTEN FROM, and it was one of these very fixtures. `USER_MESSAGE_STALE` set
  // `refutedQuote: 'unknown mode'` while its cited evidence printed `printf 'unknown mode\n' >&2; exit
  // 2` — so the evidence REPRODUCED that fragment verbatim. C-019 makes `claim` the REFUTED side and
  // `evidence` the REFUTING side; evidence that prints the quoted words back is the strongest possible
  // CONFIRMATION of them. The record's own `statement` admitted the real defect was an OMISSION — "the
  // code also exits 2, which the message never mentions" — which is not a contradiction of "unknown
  // mode" at all. A C-019 fixture violating C-019's semantics is worse than no fixture: it teaches the
  // wrong shape to everyone who copies it.
  //
  // WHY THE CHECK IS VERBATIM CONTAINMENT AND NOTHING CLEVERER. "Does this evidence refute this text?"
  // is a judgement, and a test that tried to make it would be inventing a rule. "Does the refuting side
  // print the refuted fragment back?" is decidable, and in that case the record is wrong however the
  // judgement would have gone. Compared through `quotesFragment` — the contract's own comparison — so
  // this cannot drift into a second, subtly different notion of "quotes" (ADR C-006).
  for (const synthetic of LEGAL_SYNTHETICS) {
    for (const [i, c] of (synthetic.contradictions ?? []).entries()) {
      const note = c.evidence?.note;
      if (typeof note !== 'string' || typeof c.refutedQuote !== 'string') continue;
      assert.ok(!quotesFragment(note, c.refutedQuote),
        `${synthetic.id}.contradictions[${i}]: the cited evidence note ${JSON.stringify(note)} REPRODUCES `
        + `the refutedQuote ${JSON.stringify(c.refutedQuote)}, so it confirms that fragment instead of `
        + 'refuting it — point the quote at the words the evidence actually contradicts (ADR C-019)');
    }
  }
});
...
test('18 · a map with no nodes yields no findings rather than throwing', () => {
  // …and an empty COVERAGE statement beside them (ADR decision F): `computeDrift` returns both, so a
  // caller can never render findings without the statement of what was established to produce them.
  assert.deepEqual(computeDrift({ nodes: [] }), {
    findings: [],
    coverage: { docSurfaces: [], established: [], withheld: [] },
  });
});
...
// ─── 28 · the contradiction must point at the claim that is WRONG (the run-4 defect) ──────────────
//
// Run 4 produced a STALE finding on `mode.prepr` that was SUBSTANTIVELY CORRECT — the over-budget
// message at `codex-gate.sh:519` really does tell the user to run `prepr --since-reviewed`, and
// `_prepr_common` really does swallow that token as `<base>` — and then wired the record's `claim` at
// `README.md:97`, a line the reviewer verified to be ACCURATE. The genuinely stale `user-message` was
// sitting on the same node, uncited. Every rule this engine had still passed: two resolving citations,
// both quoted, both deep-matching the node's own records, and a true statement.
//
// So `drift.json` sent a maintainer to fix a correct line. That is a distinct failure from a wrong
// verdict — right finding, wrong pointer — and the record now has to say which text the evidence
// refutes, so the pointer can be checked against the claim it names.
//
// The split between this file and `validate.mjs` is deliberate and stated in `staleFindings`: the
// CONTRACT is the validator's (a record that never names the refuted text is refused there, and
// `render.mjs` validates before it computes anything). What the drift engine owes is narrower and
// absolute — it must never EMIT a finding it can already see is misdirected.

const REFUTED_ON_STDOUT = () => node('outcome', 'refuted quote', {
  claims: [
    docClaim({ line: 7, text: 'the docs describe the outcome accurately' }),
    commentClaim({ line: 16, text: '# emits PASS on stdout' }),
  ],
  evidence: [codeEvidence({ line: 18, note: "printf 'PASS\\n' >&2" })],
  contradictions: [{
    claim: commentClaim({ line: 16, text: '# emits PASS on stdout' }),
    evidence: codeEvidence({ line: 18, note: "printf 'PASS\\n' >&2" }),
    refutedQuote: 'on stdout',
    statement: 'The comment says PASS goes to stdout; the code redirects it to stderr.',
  }],
});

test('28 · a contradiction whose refutedQuote is NOT in the claim it names FAILS CLOSED — a misdirected finding is never emitted', () => {
  const misdirected = REFUTED_ON_STDOUT();
  // The run-4 shape exactly: keep the true statement and the real refuted text, and re-point the
  // record at the node's ACCURATE claim. Nothing else changes.
  misdirected.contradictions[0].claim = docClaim({ line: 7, text: 'the docs describe the outcome accurately' });

  let thrown = null;
  assert.throws(() => computeDrift(plus(misdirected)), (e) => { thrown = e.message; return true; },
    'the engine can see the pointer is wrong — the quote it says is refuted is not in the claim it cites');
  assert.match(thrown, /outcome\.refuted_quote\.contradictions\[0\]\.refutedQuote/);
  assert.match(thrown, /on stdout/, 'the refusal must quote the text that did not match');

  // …and `validate()` refuses the same map, for the same reason. Rule 14: what a consumer refuses,
  // the validator reports.
  const res = validate(plus(misdirected), { repoRoot: REPO_ROOT });
  assert.equal(res.ok, false);
  assert.match(res.errors.join('\n'), /refutedQuote/);
});

test('28b · a correctly-wired contradiction cites the REFUTED claim first, and carries the refuted text into drift.json', () => {
  const m = plus(REFUTED_ON_STDOUT());
  assert.deepEqual(validate(m, { repoRoot: REPO_ROOT }).errors, [], 'precondition: the wired record is legal IR');

  const stale = forNode(m, 'outcome.refuted_quote').find((f) => f.class === 'STALE');
  assert.ok(stale, 'the record still raises STALE — this rule may not make a genuine contradiction harder to record');
  assert.equal(stale.citations[0].path, SH);
  assert.equal(stale.citations[0].line, 16, 'citation ONE is the claim that is wrong — the text a maintainer must change');
  assert.equal(stale.citations[1].line, 18, 'citation TWO is the observation that refutes it');
  assert.equal(stale.refutedQuote, 'on stdout',
    'and the finding carries the refuted fragment, so drift.json names the wrong text and not merely its line');

  // The reader must actually be SENT there: the property spans derivation and rendering, so it is
  // asserted end to end rather than trusted at the seam.
  const md = toMarkdown(m, findingsOf(m));
  const staleBlock = md.slice(md.indexOf('`STALE`** — `outcome.refuted_quote`'));
  assert.ok(staleBlock.startsWith('`STALE`'), 'the STALE finding must reach the report at all');
  assert.match(staleBlock.slice(0, 600), new RegExp(`${SH.replace(/[.]/g, '\\.')}\`:16`),
    'the rendered finding must cite the line that needs changing');
});

test('28c · a record predating the rule still raises STALE — the DERIVATION never weakens; the CONTRACT is the gate', () => {
  // The rule arrived after maps were already written (run 3's committed artifact among them). The
  // drift engine keeps deriving STALE from such a record exactly as before — refusing it here would
  // silently delete findings from every map that predates the field, which is the one thing a drift
  // engine may never do. `validate()` is where the requirement bites, and `render.mjs` validates
  // before it computes drift, so nothing SHIPS uncontracted.
  const legacy = REFUTED_ON_STDOUT();
  delete legacy.contradictions[0].refutedQuote;

  const m = plus(legacy);
  const stale = forNode(m, 'outcome.refuted_quote').find((f) => f.class === 'STALE');
  assert.ok(stale, 'a legacy record still accuses');
  assert.equal(stale.citations.length, 2);
  assert.ok(!('refutedQuote' in stale), 'and claims nothing about a refuted fragment it never named');

  assert.equal(validate(m, { repoRoot: REPO_ROOT }).ok, false, 'while the contract refuses to accept it');
});

// ─── 29 · the harvest reasons say what the RECORD holds, never what the world did ────────────────

/**
 * `tiny` with `env.tiny_debug`'s attestation replaced — it is the fixture's ONE harvest-eligible node
 * (evidenced, claimed only by a code comment), so each shape below yields exactly one withheld entry
 * and the reason under test is unambiguous.
 */
function withheldReason(mutate) {
  const map = load();
  mutate(nodeOf(map, 'env.tiny_debug'));
  const { coverage } = computeDrift(map);
  assert.equal(coverage.withheld.length, 1, 'fixture precondition: exactly one verdict is withheld');
  return coverage.withheld[0].reason;
}

test('29 · a withheld reason states what the ATTESTATION records, never an act nothing here can observe', () => {
  // ADR C-018's amendment (2026-08-14) settled what this gate actually knows. `docHarvest.searched`
  // and `sources[].role` are two lists the SAME extractor wrote, and every rule in `harvestStateOf`
  // compares them against each other or against the map's own `coverage`. NOTHING OPENS A FILE. An
  // extractor that read every documentation surface and one that read none produce byte-identical
  // records, so no reason derived here can distinguish them — and these reasons did exactly that:
  // *"no documentation harvest was run for this node"* and *"the harvest searched 0 of 1 declared
  // documentation surfaces, leaving 1 unread"*. Both report an ACT. The pipeline observed no act.
  //
  // This is not pedantry about verbs. These strings are the one column of the coverage table rendered
  // as PROSE rather than as a code span (see `harvestStateOf`'s path-free note), so they read as the
  // report's own voice rather than as quoted extractor data — which is precisely why they may not
  // claim more than the pipeline can see. `SKILL.md` already tells the extractor that nothing
  // downstream checks whether it read anything; a report that then tells the READER a search "was
  // run" contradicts the skill's own account of itself.
  //
  // The third reason — a map declaring no `role:"doc"` source — is NOT exercised here because it
  // already describes only the declaration ("the map declares no role:\"doc\" source"), which is a
  // fact about the map's own text. It needed no correction, and asserting over it would suggest it
  // had one.
  const cases = [
    ['no record at all', (n) => { delete n.docHarvest; }],
    ['a record naming no surface', (n) => { n.docHarvest = { searched: [], candidates: [] }; }],
  ];
  for (const [what, mutate] of cases) {
    const reason = withheldReason(mutate);
    for (const [claim, forbidden] of [
      ['that a harvest was performed, or was not', /harvest (?:was run|ran)\b|\bwas run\b/i],
      ['that a search was performed', /\bthe search ran\b|\bsearch was run\b/i],
      ['that a file went unread', /\bunread\b/i],
    ]) {
      assert.doesNotMatch(reason, forbidden,
        `${what}: the reason may not assert ${claim} — nothing here opens a file `
        + `(got ${JSON.stringify(reason)})`);
    }
    // …and it must still be ACTIONABLE. Refusing to overclaim is not licence to say nothing: the
    // record is the thing that fell short, and the reason has to name it or the coverage table
    // becomes a column of shrugs.
    assert.match(reason, /attestation|record/i,
      `${what}: the reason must still name the record as what fell short (got ${JSON.stringify(reason)})`);
    // PATH-FREE, unchanged: a source-derived string in this cell would have to be a backticked span,
    // turning the sentence into a wall. The surfaces have their own columns.
    assert.doesNotMatch(reason, /\//,
      `${what}: a source path here would break the Markdown cell (got ${JSON.stringify(reason)})`);
  }
});
