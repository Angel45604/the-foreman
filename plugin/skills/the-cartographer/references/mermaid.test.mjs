...
/**
 * ADR C-018 — what a synthetic must carry before UNDOCUMENTED can fire on it, in THREE halves.
 *
 * Completeness is DERIVED, by comparing a node's `searched` against the map's own declared
 * `role: "doc"` sources. So a map declaring none puts every node in state 3 — nowhere to look is not
 * the same as nothing to find — and a node with no record is state 3 on its own map's evidence.
 * The third statement is `coverage`: `sources[]` says the surface EXISTS and is hashed, and only
 * `coverage.read` says this run opened it in full. A harvest of a surface the map does not classify as
 * read is a search of a file the map itself does not say it read, which is state 3 as well. No one of
 * the three alone reaches state 2, which is why all three appear together everywhere below.
 */
...
function driftNode(id, kind, label, lane, cls) {
  const base = {
    id, kind, label, lane, inferred: false, evidence: [], claims: [], docHarvest: HARVESTED,
  };
  if (cls === 'PHANTOM') {
...
test('7 · a classDef per drift class present, assigned to the affected nodes', () => {
  const map = {
    sources: DOC_SOURCES, // ADR C-018 — the surface `driftNode`'s harvest records having read.
    coverage: READ_IN_FULL, // …and the map's own statement that it WAS read in full.
    nodes: [
