...
- Run tests from the repo root: `node --test "plugin/skills/the-foreman/references/*.test.mjs"` (the bare-directory form fails under Node v24 — MODULE_NOT_FOUND on the directory arg). **The FULL suite must be green at every commit.** Where a pre-existing integration assertion pins markup, tokens, or file contents that the current task legitimately changes, update that assertion IN THE SAME TASK — the files this bites are `templates.test.mjs`, `render.test.mjs`, `markdown.test.mjs`, and `contract-drift.test.mjs` (Tasks 4b and 8 in particular: 4b updates every templates/render assertion pinning old block class markup; 8 updates render.test.mjs assertions pinning old CSS tokens/content). Never delete a test without a replacement pinning the same property.
...
- [ ] **Step 2:** `node --test "plugin/skills/the-foreman/references/*.test.mjs"` → ALL PASS, zero skips.
...
What actually shipped after the pre-PR reviews (items 1–4 from round 1 — the two P1s were filed against the plan text above — item 5 from a later code-gate round; this record reflects the as-built behavior):

1. **Font refactor** (review blocker: the over-budget, unreviewable stylesheet). `style.css` no longer embeds the two base64 woff2 payloads. Its `@font-face` `src` values carry the placeholders `__FONT_SORA_B64__` / `__FONT_NUNITO_B64__` (OFL notice kept in the sheet); `render.mjs` reads `references/fonts/sora-latin.woff2` + `nunito-sans-latin.woff2` at assembly, base64-embeds them into the placeholders, and FAILS LOUDLY on a missing placeholder or unreadable font file. The stylesheet dropped from 120,787 to 45,903 bytes; the assembled page is byte-identical to the pre-refactor render (sha-verified against `gate-board-ledger.json`).
2. **Document shell — dual output** (review P1, adapted for a constraint the reviewer could not see: the hosted-Artifact publish contract REQUIRES the `outPath` file to ship with NO doctype/html/head/body wrapper — the host supplies them). `outPath` therefore stays shell-less (now test-pinned), and `render.mjs` ADDITIONALLY writes a `<stem>.local.html` sibling: a complete standards-mode document (`<!doctype html>`, `lang="en"`, `<meta charset>`, viewport, `<title>`) wrapping the SAME assembled content in `<body>` for the local Chrome fallback's charset/viewport correctness. One fail-closed secret scan covers both (the local variant is a pure wrapper of already-scanned content); `render()` returns `localPath`/`localBytes`, and `open-artifact.mjs` prefers the `.local.html` sibling when present.
3. **askShape hardening** (review P1). `askShape` accepts `meta.ask` ONLY when `typeof headline === 'string' && headline.trim() !== ''` — an object without one (`{}`, `{note:'x'}`, `{headline:''}`) used to win the nullish pick, blank the ask strip, and drop `decision.question`. Both copies (templates.mjs + the markdown.mjs mirror — two by design, cross-referenced) are tightened identically; `lint.mjs` gained `lint: malformed-ask meta`, firing whenever `meta.ask` is present but fails the shape.
4. **keyStats coexistence — the both-present rule** (review P2). `meta.keyStats` wins the hero tiles; when BOTH `meta.keyStats` and a type's derived stats exist (`dashboard.stats`, liveRun's synthesized cost/blast-radius pair), the derived stats render as VISIBLE stat wells (a `statRow` block) inside the type's unit body instead of being dropped. The twin serializes the hero keyStats in the head and the derived stats within the type's section. (When `meta.keyStats` is ABSENT the derived stats are PROMOTED to the hero, and the twin mirrors that too: the promoted stats serialize as the head's hero list, before the ask, exactly once.)
5. **Recommended-marker source — the effective ask** (a later prepr code-gate round, commit `2b48b97`; SUPERSEDES the Task 6/7 sentences above that keyed the option-card marker off `decision.recommendation` — those sentences now carry AMENDED AS SHIPPED notes pointing here). As shipped, `optionCards` badges the option whose label matches the **EFFECTIVE ask's** recommendation, so the badge always agrees with the single `.rec` strip: a valid `meta.ask` override MOVES the badge (the board can never badge A while the strip says B), an overriding ask WITHOUT a recommendation badges nothing (`askChapterHtml` passes `effectiveAsk.recommendation ?? null` so the default parameter never resurfaces the overridden `decision.recommendation`), a recommendation matching no option label badges nothing, and only the orphan content-labeled chapters (no effective ask at all) fall back to the decision's own recommendation — the only source there is. Test-pinned in `templates.test.mjs` ("the Recommended badge follows the EFFECTIVE ask").
